...
void pltfm_init(void) {
    memset(comRxHandler, 0, sizeof(comRxHandler));
    memset(comRxBytes, 0, sizeof(comRxBytes));
    if(cb.call == NULL) {
		cb.call = pltfm_call;
		cb.next = callbacks;
...
void pltfm_call(void) {
    int t;
    for(t = 0; t < COM_PORTS; t++) {
        if(comRxHandler[t] != NULL && comRxBytes[t] > 0) {
            if(UART_buffer(t) >= comRxBytes[t]) execute_function(comRxHandler[t]);
        }
    }
}
...
void sf_spiBlock(void) {
    ...
    if(d1.val.i >= 0 && d2.val.i < SPI_PORTS) {
